...
if (params.help) {
    ...
    log.info "--metadata	    FILE		 File with all informations (columns) about samples (rows), excel format (xlsx)"
...
process RDS_preprocessing {

	label 'mid_mem'

	publishDir params.output_folder+"/RDS_preprocessing/", mode: 'copy'
        cpus params.threads
	...
	shell :
	'''
	shiny_preprocessing.R !{otuTable} !{otuTax} !{asvTable} !{asvTax} !{params.metadata} !{params.experiment}
	'''
}
